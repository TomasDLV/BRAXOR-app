...
import { deleteCategory, type ActionState } from "@/actions/categoryActions";
...
function DeleteButton({ disabled }: { disabled?: boolean }) {
  ...
  return (
    <button
      ...
      title={disabled ? "Tiene productos asignados" : "Eliminar categoría"}
      ...
      onClick={(e) => {
        if (!confirm("¿Eliminar esta categoría?")) e.preventDefault();
      }}
    >
      {pending ? (
...
export default function DeleteCategoryButton({
  ...
  const [, formAction] = useActionState<ActionState, FormData>(deleteCategory, null);
